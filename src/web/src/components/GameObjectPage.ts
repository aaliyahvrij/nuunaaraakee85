...
import { updateGameObject, deleteGameObject, fetchGameObjects } from "../services/gameObjectService";

@customElement("gameObjectPage")
export class GameObjectPage extends LitElement {
    ...
    private handleEditClick(gameObject: GameObjectFormResult): void {
        ...
        const newPrice: string | null =
            gameObject.type === "item"
                ? prompt("Enter new price:", gameObject.price?.toString() ?? "")
                : null;
        const newHp: string | null =
            gameObject.type === "character" ? prompt("Enter new HP:", gameObject.hp?.toString() ?? "") : null;
                ...
                <tbody>
                    ${this.gameObjects.map(
                        (gameObject) => html`
                            ...
                            <td>
                                <button @click=${(): void => this.handleDeleteClick(gameObject.id)}>
                                    Delete
                                </button>
